...
![](doc/quarto_render.png){width=75%}
...
In the YAML header example above, the `.qmd` file is set to create an .html document when it is rendered. The default is for Quarto to create a subdirectory named `_files` to contain the images, javascript, etc. that make the .html file pretty. If, like me, you'd rather not deal with this bundle of files, you can set an extra flag to ask Quarto to create self-contained .html files like this:
...
If you would prefer a .pdf output document, you can change the format parameter in your YAML header to `pdf`, which uses LaTex (note: you must have LaTex already installed on your computer). You can download / install LaTex here:
...
Chunk output can be customised with **options**, arguments supplied at the start of your code chunk, and designated using `#|`. The `knitr` package provides almost 60 options that you can use to customize your code chunks. Here we'll cover the most important chunk options that you'll use frequently. You can see the full list at <https://quarto.org/docs/reference/cells/cells-knitr.html>.
...
You can embed images that are stored on your hard drive by using this syntax: `![optional caption text](path/to/img.png)` where the path to image is a path relative to wherever your .qmd file is stored. For example, the line:
...
`![Have we strayed too far from our animal selves?](doc/PhD_coding.png){width=50% fig-align="left"}`
...
The best way to learn the formatting for Quarto is simply to practice. It will take a few days, but soon the format will become second nature, and you won't need to think about it. Here is a handy Quarto cheatsheet made by Posit. Here is the html version: <https://rstudio.github.io/cheatsheets/html/quarto.html> and here is the pdf version: <https://rstudio.github.io/cheatsheets/quarto.pdf>
...
Quarto tutorial: In this tutorial, you can click Positron and R at the top of the page to get a specific tutorial for Positron / R users. Then you can toggle to other IDEs / python if you want to get familiar with Quarto for those use cases: <https://quarto.org/docs/get-started/hello/rstudio.html>
