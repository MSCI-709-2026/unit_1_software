...
Remember that R is the programming language we will learn in this course. However, when we write and run code we will usually do it through Positron. Positron is a free, open source multi-language Integrated Development Environment (IDE). It provides a built in editor, works on all platforms (including on servers) and provides many advantages like integration with gitHub and software for writing dynamic notebooks such as Quarton and RMarkdown files. You'll learn about this later. You may have heard of RStudio; Positron is the next generation of RStudio. It is forked from the open source version of Microsoft's VS Code (an editor for any programming language), and Positron is configured for both R and Python coding. To get started, open Positron just as you would open any other application on your computer.

When you open Positron, a window should appear in your screen that looks something like this:

![](doc/positron_start_session.png)

When you first open Positron, there are three large panes. The lower portion of the main center pane has the `CONSOLE` tab, where you can directly type in the code you want to run. The output from your code is also printed directly into the console. Depending on your system, and if you've run a previous session in `Positron`, you might need to start this session by clicking `Start Session` in the `CONSOLE` tab (or in the upper right hand corner) and selecting the most recent version of R installed on your computer. (Note that if you have Python on your computer, you could select Python instead to start coding in Python.) Now the other step to get started is to select a folder that represents your workspace for this coding session. Select the folder on your local hard drive (or make a new one) where you will store all of your files for Unit 1 in this class.

The upper right pane shows the active Positron `SESSION`, which lists all of the objects that are currently loaded; this pane can also be toggled to the `HELP` tab where you can search for and read documentation for R functions and packages. The lower right pane can be used to view `Plots` you have made. A working R session will look something like this:

![](doc/positron_panes.png)

It is easier on my eyes to use a dark screen, so the first thing I would do is go to the `Positron` menu in the upper left corner, then `Settings -> Themes -> Color Theme`. I like the default dark theme but you should use whatever is most comfortable for you.

To start programming, type R code into the bottom line of the Positron `CONSOLE` pane and then hit `Enter` to run it. The code you type is called a command, because it will command your computer to do something for you. The line you type it into is called the command line. When you type a command at the prompt and hit `Enter`, your computer executes the command and shows you the output (e.g., results). Then Positron displays a fresh prompt for your next command.
...
If you type 1 + 1 into the Positron console and hit `Enter`, R will interpret the command like a calculator and display the output:

```{r}
1+1
```

However, if you type an incomplete command and press `Enter`, R will display a + prompt, which means R is waiting for you to type the rest of your command. Either finish the command or hit `control + C` to start over:
...
Sometimes you'll run a command that takes FOREVER, and perhaps you realize that's because you asked R to do something you shouldn't have, or perhaps you just don't want to wait around for the command to finish because you are late for coffee with your BFF. With your cursor in the console pane, hit the `control + C` keys and R will stop running the command and give you the `>` prompt again. Sometimes it will take a minute for R to respond.

### Creating an R script

When you need to write more than one or two lines of code, typing your commands into the console becomes very impractical. Instead, we will open a new R script (.R file), which is a type of text file where we can save every line of code that we write. Go to `File` -\> `New File...` -\> `R File` to get started. Now your new blank R script file will appear in the center pane above the `CONSOLE` pane.

You can still type commands into the console whenever you would like to. However, you can also type your commands into the R script, and when you run the commands, the commands and output will be printed to the console. Try typing `print('Hello world')` as the first line in your R script. To run or "execute" your command, put your cursor anywhere on the line and hit `control + Enter` in Windows, `command + Enter` on a Mac or `control + Return` on a Linux machine. The cursor will automatically advance to the next line so that you can run your code line by line. If you highlight a section of code, the same run command will run all of the lines within the highlighted section. Alternatively, with your cursor on the line you want to run, or after highlighting the selection of lines you want to run, you can right-click on the script pane and select `Execute Code`. Now you can edit and save your code in the R script - that way you can look at it or re-run it later.
...
To see what built-in constants R stores, we can consult Positron's *HELP* features. We can access the help feature from the Console by typing a `?`. Let's try using *Help* to see which constants R stores, then let's use it to learn about the `print()` function that we used earlier:

```{r}
?Constants
?print
?constants # does not have an exact match
??constants 
```

When using the help feature, you use a single question mark to bring up the documentation on a function in a loaded package whose name you know exactly. Use two question marks for a broader search for a term across all installed packages.

Now that we have explored what constant values are programmed into base R, note that if we had called our own variable with the name `pi` it would have overwritten the built-in R constant:
...
It’s also best to avoid periods (.) within a variable name as in my.dataset. Historically, there are many functions in R with periods in their name, but since periods have a special meaning in R, it’s better to not use them and instead use underscores (\_).
...
That’s it. R will have your computer visit the website, download ggplot2, and install the package in your hard drive right where R looks for it. We'll be using plenty of packages in this class, with special emphasis on the suite of packages in the `tidyverse` such as `ggplot2`. Once you have a package installed on your computer, you'll need to load it at the top of your script with the `library()` function. For example:
...
There is a great (free!) beginner's guide to programming in R: “Hands-On Programming with R” by Garrett Grolemund <https://rstudio-education.github.io/hopr/>

The recommended textbook for the class is "R for Data Science": <https://r4ds.hadley.nz/>