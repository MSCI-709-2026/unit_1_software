...
![](doc/positron_intro.png)

When you first open Positron, there are three large panes. The lower portion of the main center pane is the `CONSOLE`, where you can directly type in the code you want to run. The output from your code is also printed directly into the console. The upper right pane shows the Positron `SESSION`, which lists all of the objects that are currently loaded; this pane can also be toggled to the `HELP` tab where you can search for and read documentation for R functions and packages. The lower right pane can be used to view `Plots` you have made. A working R session will look something like this:
